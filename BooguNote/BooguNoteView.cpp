...
void CBooguNoteView::OptimizeSendWM_NCPAINT()
{
    if (!m_bDirty)
	{
		m_bDirty = true;
...
void CBooguNoteView::OptimizeSendWM_PAINT()
{
...
LRESULT CBooguNoteView::OnPaint(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
{
    ...
    //CDC dc(realDC.m_hDC);  //hunter 测试使用
...
{
	//hunter_test_WM_NCPAINT   GetParent().SendMessageW(WM_NCPAINT);
	bHandled = FALSE;
			...
			if ((*ppTextBlock)->m_bShown)//查找所有可见的文字块
			{
				...
				else //left click without associated key 没有按键, 单纯点击
				{
					if ((*ppTextBlock)->HitTest(x, y)/* && */)
					{
						...
						else
						{
							...
							if (m_pTextBlockFocused->m_bFile && WM_LBUTTONDBLCLK == uMsg)
							{
								...
								{
									if(PathIsDirectory(szTempCache))
									{
									    ShellExecute(NULL, _T("open"), _T("explorer.exe"), szTempCache, NULL, SW_MAXIMIZE);  
									}
									else
									{
									    ShellExecute(NULL, NULL, szTempCache, NULL, NULL, SW_SHOW);
					...
					else if ((*ppTextBlock)->m_pTextHandle->HitTest(x, y))
					{
						...
						{
							CloseBlocks(index);
							if(g_config.bEnterSHRINK_SELF && TEXT_BLOCK_SHRINK !=((*ppTextBlock)->m_nExpandState)) //修正Bug, 已经是收缩状态不再重复收缩. By hunter 2014-12-6 23:32:38
							{
						...
						{
							ExpandBlocks(index);
							if(g_config.bEnterSHRINK_SELF && TEXT_BLOCK_NARROW !=((*ppTextBlock)->m_nExpandState)) //修正Bug, 已经是展开状态不再重复展开. By hunter 2014-12-6 23:32:38 
							{
...
{	
	...
	for(vector<CBooguNoteText*>::iterator ppTextBlock = m_textBlockList.begin(); ppTextBlock != m_textBlockList.end(); ++ppTextBlock)
	{
		...
		(*ppTextBlock)->m_pTextHandle->y = nBlockStart_Y + nTextHandleOffsetY;
		CRect* prc = NULL;
		...
		{
			//text only 1 line, so show icon and handle in the same line.
			...
			nTextBlockIndent = (*ppTextBlock)->m_pTextIcon->x + (*ppTextBlock)->m_pTextIcon->width +3;
		...
		prc->MoveToXY(nTextBlockIndent,nBlockStart_Y);//hunter : 平移文字块区域 
...
LRESULT CBooguNoteView::OnRBottonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (bHited)
	{
		...
		CalcLayout();
		CalcScrollBar();
		OptimizeSendWM_PAINT(); //flush size into text block and repaint
	}
	else
	{
		m_bRButtonDown = true;
...
LRESULT CBooguNoteView::OnSize(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM &lParam, BOOL& bHandled)
{
	...
	{	
	    ...
	    g_config.narrowWidth = rcWindow.Width()-(g_config.ptStart.x + g_config.nHandleWidth+2+1+ g_config.nScrollbarWidth+1+16+2+FILE_ICON_WIDTH+3);//16是图标的大小对应 (*ppTextBlock)->m_pTextIcon->width
	}
...
int CBooguNoteView::SaveAs()
{
	...
	CFileDialog fd(FALSE, _T("boo"), fm,   OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST, strFilter);
	//fd.m_ofn.Flags = OFN_DONTADDTORECENT|OFN_HIDEREADONLY|OFN_FILEMUSTEXIST;
	fd.m_ofn.lpstrInitialDir = g_config.szRootStorageDir;
	if (IDCANCEL == fd.DoModal())
	{
...
LRESULT CBooguNoteView::OnUserRequestResize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CBooguNoteText* pTextBlock = (CBooguNoteText*)wParam; 
	pTextBlock->GetTextServices()->TxSendMessage(EM_SETCHARFORMAT, SCF_DEFAULT, (LPARAM)&pTextBlock->cf, 0);
		...
		if (IDOK == fd.DoModal())
		{
			...
			for(int i=0; i<moveList.size(); ++i)
			{
				...
				if (filePathBeforeMove != filePathAfterMove)
				{
					...
					ZeroMemory (szFrom, sizeof(TCHAR)*1024*1024);
					_stprintf(szFrom, filePathBeforeMove.GetBuffer(0));
					filePathBeforeMove.ReleaseBuffer();
					TCHAR* szTo = new TCHAR[1024*1024];
					ZeroMemory (szTo, sizeof(TCHAR)*1024*1024);
					CString tempFolderPath = folderPath;
					if (tempFolderPath.Right(1) == _T('\\'))
					{
						tempFolderPath.TrimRight(_T('\\'));
					}
					_stprintf(szTo, tempFolderPath.GetBuffer(0));
					...
					参见 http://msdn.microsoft.com/en-us/library/windows/desktop/bb759795(v=vs.85).aspx */
					shfo.pTo = szTo;
					shfo.pFrom = szFrom;
					...
					if (shfo.fAnyOperationsAborted == 0 && iRC ==0)
					{
						...
						moveList[i]->GetTextServices()->TxSendMessage(WM_SETTEXT, 0, (LPARAM)moveList[i]->m_szCache, 0);
					}
					else
					{
						break;
...
LRESULT CBooguNoteView::OnStateNarrowAll(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    for (int i=0; i<m_textBlockList.size(); ++i)
    {
        ...
        {
            ExpandBlocks(i);
			if(g_config.bEnterSHRINK_SELF && TEXT_BLOCK_NARROW !=(m_textBlockList[i]->m_nExpandState)) //修正Bug, 已经是展开状态不再重复展开. By hunter 2014-12-6 23:32:38 
			{
			    NarrowPointTextBlock(m_textBlockList[i]);
			}            
        }       
        m_bDirty = true;
    }
...
LRESULT CBooguNoteView::OnStateShrinkAll(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    for (int i=0; i<m_textBlockList.size(); ++i)
    {
        ...
        {
            CloseBlocks(i);
			if(g_config.bEnterSHRINK_SELF && TEXT_BLOCK_SHRINK !=(m_textBlockList[i]->m_nExpandState)) //修正Bug, 已经是收缩状态不再重复收缩. By hunter 2014-12-6 23:32:38
			{
			    ShrinkTextBlock(m_textBlockList[i]);
			}            
        }        
        m_bDirty = true;
    }
