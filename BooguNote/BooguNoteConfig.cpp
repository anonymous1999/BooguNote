...
void CBooguNoteConfig::ResetConfig()
{
	...
	bEnterCHANGE_TO_RELATIVEPATH   = true;
	bEnterCHANGE_TO_RELATIVEPATH = false;
...
void CBooguNoteConfig::WriteConfigToIni()
{
	...
	BinaryToString((BYTE*)&bEnterCHANGE_TO_RELATIVEPATH, sizeof(bEnterCHANGE_TO_RELATIVEPATH), buf);
	WritePrivateProfileString(_T("TextBlock"),_T("EnterCHANGE_TO_RELATIVEPATH"),buf,strConfigFileName);	
	
	BinaryToString((BYTE*)&bEnterSHOW_RAW_PATH, sizeof(bEnterSHOW_RAW_PATH), buf);
	WritePrivateProfileString(_T("TextBlock"),_T("EnterSHOW_RAW_PATH"),buf,strConfigFileName);		
	...
	WritePrivateProfileString(_T("MainFrame"),_T("Version"),_T("0.3.3.12"),strConfigFileName);
	...
	_itot(nCAPTION_CHANGE_RULE, buf, 10);
	WritePrivateProfileString(_T("MainFrame"),_T("CaptionChangeRule"),buf,strConfigFileName);		
...
void CBooguNoteConfig::ReadConfigFromIni()
{
	...
	GetPrivateProfileString(_T("TextBlock"),_T("EnterCHANGE_TO_RELATIVEPATH"),_T("01"),buf, sizeof(bool)*2+1,strConfigFileName);
	StringToBinary(buf, sizeof(bool)*2+1, (BYTE*)&bEnterCHANGE_TO_RELATIVEPATH);
	
	GetPrivateProfileString(_T("TextBlock"),_T("EnterSHOW_RAW_PATH"),_T("00"),buf, sizeof(bool)*2+1,strConfigFileName);
	StringToBinary(buf, sizeof(bool)*2+1, (BYTE*)&bEnterSHOW_RAW_PATH);			
	...
	GetPrivateProfileString(_T("MainFrame"),_T("CaptionChangeRule"), _T("0"),buf, 100, strConfigFileName);
	nCAPTION_CHANGE_RULE = _tstoi(buf);	
